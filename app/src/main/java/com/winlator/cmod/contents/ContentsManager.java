...
public class ContentsManager {
    ...
    public static final String[] DXVK_TRUST_FILES = {"${system32}/d3d8.dll", "${system32}/d3d9.dll", "${system32}/d3d10.dll", "${system32}/d3d10_1.dll", "${system32}/d3d10core.dll", "${system32}/d3d11.dll", "${system32}/dxgi.dll", "${syswow64}/d3d8.dll", "${syswow64}/d3d9.dll", "${syswow64}/d3d10.dll", "${syswow64}/d3d10_1.dll", "${syswow64}/d3d10core.dll", "${syswow64}/d3d11.dll", "${syswow64}/dxgi.dll"};
    public static final String[] VKD3D_TRUST_FILES = {"${system32}/d3d12core.dll", "${system32}/d3d12.dll", "${syswow64}/d3d12core.dll", "${syswow64}/d3d12.dll"};
    ...
    public interface OnInstallFinishedCallback {
        void onFailed(InstallFailedReason installFailedReason, Exception exc);

        void onSucceed(ContentProfile contentProfile);
    }

    public enum ContentDirName {
        CONTENT_MAIN_DIR_NAME("contents"),
        CONTENT_WINE_DIR_NAME(ContentProfile.MARK_WINE),
    ...
    public ContentsManager(Context context) {
        ...
        this.preferences = context.getSharedPreferences("contents_manager_prefs", 0);
    }

    public void setGraphicsDriverInstalled(String driverVersion, boolean installed) {
        this.preferences.edit().putBoolean("graphics_driver_installed_" + driverVersion, installed).apply();
    }

    public void setRemoteProfiles(String json) {
        try {
            this.remoteProfiles = new ArrayList<>();
            ...
            for (int i = 0; i < content.length(); i++) {
                try {
                    ...
                    remoteProfile.type = ContentProfile.ContentType.getTypeByName(object.getString(ContentProfile.MARK_TYPE));
                    remoteProfile.verName = object.getString("verName");
                    remoteProfile.verCode = object.getInt("verCode");
                    this.remoteProfiles.add(remoteProfile);
    ...
    public void syncContents() {
        ...
        for (ContentProfile.ContentType contentType : ContentProfile.ContentType.values()) {
            this.profilesMap.put(contentType, new LinkedList());
        }
        for (ContentProfile.ContentType type : ContentProfile.ContentType.values()) {
            List<ContentProfile> profiles = this.profilesMap.get(type);
            File typeFile = getContentTypeDir(this.context, type);
            ...
            if (this.remoteProfiles != null) {
                Iterator<ContentProfile> it = this.remoteProfiles.iterator();
                while (it.hasNext()) {
                    ContentProfile remote = it.next();
                    if (remote.type == type) {
                        boolean exists = false;
                        Iterator<ContentProfile> it2 = profiles.iterator();
                        while (true) {
                            if (!it2.hasNext()) {
                                break;
                            }
                            ContentProfile profile2 = it2.next();
                            if (profile2.verName.equals(remote.verName) && profile2.verCode == remote.verCode) {
                                exists = true;
                                break;
                            }
                        }
    ...
    public void extraContentFile(Uri uri, OnInstallFinishedCallback callback) {
        cleanTmpDir(this.context);
        File file = getTmpDir(this.context);
        boolean ret = TarCompressorUtils.extract(TarCompressorUtils.Type.XZ, this.context, uri, file);
        if (!ret) {
            ret = TarCompressorUtils.extract(TarCompressorUtils.Type.ZSTD, this.context, uri, file);
        }
        if (!ret) {
        ...
        String imagefsPath = this.context.getFilesDir().getAbsolutePath() + "/imagefs";
        for (ContentProfile.ContentFile contentFile : profile.fileList) {
            ...
            if (!isSubPath(imagefsPath, realPath) || isSubPath(getContentDir(this.context).getAbsolutePath(), realPath) || realPath.contains("dosdevices")) {
    ...
    public void finishInstallContent(ContentProfile profile, OnInstallFinishedCallback callback) {
        File installPath = getInstallDir(this.context, profile);
        if (installPath.exists()) {
            ...
            if (!getTmpDir(this.context).renameTo(installPath)) {
                callback.onFailed(InstallFailedReason.ERROR_UNKNOWN, null);
            }
            callback.onSucceed(profile);
        }
    ...
    public List<ContentProfile> getProfiles(ContentProfile.ContentType type) {
        if (this.profilesMap != null) {
            return this.profilesMap.get(type);
        }
        return null;
    }
    ...
    public List<ContentProfile.ContentFile> getUnTrustedContentFiles(ContentProfile profile) {
        ...
        for (ContentProfile.ContentFile contentFile : profile.fileList) {
            if (!this.trustedFilesMap.get(profile.type).contains(Paths.get(getPathFromTemplate(contentFile.target), new String[0]).toAbsolutePath().normalize().toString())) {
    ...
    private boolean isSubPath(String parent, String child) {
        return Paths.get(child, new String[0]).toAbsolutePath().normalize().startsWith(Paths.get(parent, new String[0]).toAbsolutePath().normalize());
    }

    private void createDirTemplateMap() {
        if (this.dirTemplateMap == null) {
            this.dirTemplateMap = new HashMap();
            String imagefsPath = this.context.getFilesDir().getAbsolutePath() + "/imagefs";
            String drivecPath = imagefsPath + "/home/xuser/.wine/drive_c";
            this.dirTemplateMap.put("${libdir}", imagefsPath + "/usr/lib");
            this.dirTemplateMap.put("${system32}", drivecPath + "/windows/system32");
            this.dirTemplateMap.put("${syswow64}", drivecPath + "/windows/syswow64");
            this.dirTemplateMap.put("${bindir}", imagefsPath + "/usr/bin");
            this.dirTemplateMap.put("${sharedir}", imagefsPath + "/usr/share");
        }
    }

    private void createTrustedFilesMap() {
        String[] paths;
        if (this.trustedFilesMap == null) {
            this.trustedFilesMap = new HashMap();
            for (ContentProfile.ContentType type : ContentProfile.ContentType.values()) {
                List<String> pathList = new ArrayList<>();
                this.trustedFilesMap.put(type, pathList);
                switch (type) {
                    ...
                    case CONTENT_TYPE_WOWBOX64:
                        paths = WOWBOX64_TRUST_FILES;
                        break;
                    case CONTENT_TYPE_FEXCORE:
                        paths = FEXCORE_TRUST_FILES;
                        break;
                    default:
                        paths = new String[0];
                        break;
                ...
                for (String path : paths) {
                    pathList.add(Paths.get(getPathFromTemplate(path), new String[0]).toAbsolutePath().normalize().toString());
                }
            }
        }
    }
    ...
    private String getPathFromTemplate(String path) {
        ...
        for (String key : this.dirTemplateMap.keySet()) {
            realPath = realPath.replace(key, this.dirTemplateMap.get(key));
        }
        return realPath;
    }

    public void removeContent(ContentProfile profile) {
        if (this.profilesMap.get(profile.type).contains(profile)) {
            FileUtils.delete(getInstallDir(this.context, profile));
            this.profilesMap.get(profile.type).remove(profile);
            syncContents();
        }
    }
    ...
    public boolean applyContent(ContentProfile profile) {
        if (profile.type != ContentProfile.ContentType.CONTENT_TYPE_WINE || profile.type != ContentProfile.ContentType.CONTENT_TYPE_PROTON) {
            for (ContentProfile.ContentFile contentFile : profile.fileList) {
                ...
                File sourceFile = new File(getInstallDir(this.context, profile), contentFile.source);
                targetFile.delete();
                FileUtils.copy(sourceFile, targetFile);
                if (profile.type == ContentProfile.ContentType.CONTENT_TYPE_BOX64) {
                    FileUtils.chmod(targetFile, 505);
                }
            }
            return true;
        }
